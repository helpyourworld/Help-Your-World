...
function com_rokcandy_initial_data_population($db, &$status) {
	...
	if ($db->getNumRows() == 0) {
	    $sql = "insert into #__categories (title, name, alias, image, section, image_position, published, ordering) values " .
	            "('Basic',     '','basic',     '','com_rokcandy','left','1','1')," .
	            "('Typography','','typography','','com_rokcandy','left','1','1')";
	    $db->setQuery($sql);
	    if (!$db->query()) {
			$status->errormsg[] = $db->getErrorMsg();
			print_r($status);
